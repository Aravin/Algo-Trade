...
//         const strategy = getMarketSentiment(globalSentiment, niftySentiment, pcr);

//         // finvasia
//         const account = Account.getInstance();
//         account.token = await upstox.login();
...
//             log.info(`Market is ${niftySentiment} ✅, placing ${strategy} Order`);
...
//             const canExit = canExitPosition(changePercent, volatility, ORDERED_SENTIMENT, niftySentiment);
...
// const placeOrder = async (strategy: 'Bear PUT Spread' | 'Bull CALL Spread' | 'Long Straddle' | 'Short Straddle' | 'Short Strangle') => {
