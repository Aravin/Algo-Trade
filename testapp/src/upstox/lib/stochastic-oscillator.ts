...
const calculateStochasticOscillator = (candles: Candle[], period: number = 14, smoothing: number = 3): { k: number, d: number } => {
  ...
  if (candles.length < period + smoothing - 1) {
  ...
  candles = candles.slice(0, period + smoothing - 1);
  ...
  // Calculate %D (EMA of %K) - you should have enough kValues now
  const dValues = ema(smoothing, kValues);
